...
const Sidebar = ({ collapsed, toggleCollapse, onHelpPopupToggle }) => {
    ...
    const tenantName = user?.tenantName || (user?.role === 'super_admin' ? 'Super Admin Mode' : 'Not Available');
    ...
    const buildMenuItems = useMemo(() => {
        ...
        // Add Admin menu for all users who can access admin features
        if (role === 'admin' || role === 'super_admin' || canUserView(role, '/users')) {
            const adminChildren = [];
            
            // Add Invoice Field Config
            // if (canUserView(role, '/configure')) {
            //     adminChildren.push({
            //         label: 'Invoice Field Config',
            //         path: '/configure',
            //         icon: <FileCog size={14} />,
            ...
            // Add Vendor Invoice Config
            if (canUserView(role, '/vendor_configure')) {
                adminChildren.push({
            ...
            // // Add Tenant Logo for tenant admins
            // if ((role === 'admin' || role === 'super_admin') && canUserView(role, '/tenantlogo')) {
            //     adminChildren.push({
            //         label: 'Tenant Logo',
            //         path: '/tenantlogo',
            //         icon: <Image size={14} />,
            ...
            // Add Tenant Registration for tenant admins
            if ((role === 'admin' || role === 'super_admin') && canUserView(role, '/tenantreg')) {
                adminChildren.push({
        ...
        if (role === 'super_admin') {
            ...
            // Add Parameters
            if (canUserView(role, '/parameters')) {
                superAdminItems.push({
                    label: 'Parameters',
                    path: '/parameters',
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                {showSidebarContent && (
                                    <div className="flex flex-col items-start leading-snug">
                                        ...
                                        <span className="text-[9px] text-[#7f8c8d] leading-tight">
                                            {tenantName}
                                        </span>
                                    </div>
                                )}
                                ...
                                <div className="bg-[#f8f9fa] p-3 rounded-t-md border-b border-[#e9ecef]">
                                    <div className="text-xs">
                                        ...
                                        <div className="flex flex-col gap-0.5">
                                            ...
                                            <span className="text-[9px] text-[#7f8c8d]">{tenantName}</span>
                                        </div>
                                    </div>
                                </div>
