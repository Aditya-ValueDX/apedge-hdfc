...
import { Trash2, Pencil, GripVertical, Plus, Save, AlertTriangle, Lightbulb, ChevronDown, Minus, SquareArrowOutUpRight, Edit3, SquareArrowOutUpRight as SquareArrowOutUpRightLucide, X, Settings } from 'lucide-react';
...
// --- Static Data ---
const fieldTypes = ['text', 'number', 'date', 'array', 'boolean', 'tab', 'button']; // Removed 'dropdown' as requested, added 'tab', 'input'
...
const DraggableRow = ({ field, activeIndex, userRole, fields }) => {
  ...
  const parentFields = fields.filter(f => {
    // Don't show the current field as its own parent
    if (f.uniqueId === field.uniqueId) return false;

    // For tabs: can have other tabs as parent
    if (field.type === 'tab') {
      return (f.type === 'tab') && f.name;
    }

    // For arrays: can only have other tabs as parent (not arrays)
    if (field.type === 'array') {
      return (f.type === 'tab') && f.name;
    }

    // For other types: can have arrays or tabs as parent
    return ((f.type === 'array') || (f.type === 'tab')) && f.name;
  });
  const parentNames = parentFields.map(f => f.name);
  ...
  return (
    <tr className="bg-accent/10 border-2 border-accent shadow-md">
      ...
      <td className="text-center px-2 py-1 text-xs align-middle min-w-[40px]">
        <button className="bg-transparent text-gray-400 border-none p-1.5 rounded-full cursor-not-allowed">
          <Trash2 size={12} />
        </button>
      </td>
    ...
    <tr className="hover:bg-accent/5 transition-colors">
      ...
      <td className="px-2 py-1 text-xs align-middle border-b border-r border-gray-200 max-w-[200px]">
        <div className="flex items-center gap-1 rounded-md p-1">
          ...
          >
            <SquareArrowOutUpRightLucide size={12} />
          </button>
        </div>
      </td>
...
function SortableRow({ field, idx, onFieldChange, canEdit, canDelete, fields, userRole, openDescriptionPopup, openCustomValuesPopup, openMetaConfigPopup, tableWrapperRef, scrollPositionRef }) {
  ...
  const validateFieldName = (fieldName) => {
    const isValid = /^[a-zA-Z0-9_]+$/.test(fieldName) && fieldName.length > 0;
    setIsFieldNameValid(isValid);

    if (!isValid && fieldName.length > 0) {
      setFieldNameErrorMessage('Only letters, numbers, and underscores allowed.');
    } else if (!isValid && fieldName.length === 0) {
      setFieldNameErrorMessage('Field name cannot be empty.');
    } else {
  ...
  const handleBlur = (key, value) => {
    ...
    if (key === 'name') {
      const isValid = validateFieldName(value);
      if (!isValid) {
        // Remove the toast error since we're now showing the error message below the input
        // toast.error('Field name can only contain letters, numbers, and underscores. No spaces or special characters allowed.');
        ...
        >
          ...
          {fields
            .filter((f) => {
              ...
              // For other types (text, number, etc.): can have arrays or tabs as parent
              return ((f.type === 'array') || (f.type === 'tab')) && f.name;
            })
            .map((f) => (
...
const VendorInvoiceConfig = () => {
  ...
  const fetchInvoiceConfig = useCallback(async () => {
    let isMounted = true;
    if (!user || !token) {
      setLoading(false);
      return;
    ...
    const isSuperAdmin = userRole === 'super_admin';
    // Using ap_vendor_field_config as per user's SQL schema
    const fetchUrl = isSuperAdmin ? `/api/v1/tables/ap_vendor_field_config` : `/api/v1/tables/ap_vendor_field_config?tenant_id=eq.${tenantId}`;
    setLoading(true);
    ...
    try {
      ...
      if (res.data && res.data.length > 0) {
        ...
        const convertedForFrontend = sortedData.map((f) => {
          ...
          return {
            vendor_config_id: f.vendor_config_id,
            uniqueId: f.vendor_config_id || generateUniqueId(),
            name: f.field_name || '',
  ...
  }, [user, userRole, tenantId, token, fetchTenantNames]);

  const fetchGenericFields = (isMounted) => {
    if (!token) {
      return;
    }
    // Using ap_generic_fields
    fetch(`/api/v1/tables/ap_generic_fields`,
      {
        headers: {
      ...
      .then((data) => {
        ...
        const mappedForFrontend = data.map((f, index) => ({
          ...
          name: f.field || '',
          display_name: toTitleCase(f.field) || '', // New mapping
          type: f.type || 'text',
          parent: f.parent || '',
          desc: f.description || '', // Updated mapping
          custom_values: f.custom_values || '',
          is_required: f.is_required || false,
          is_visible: f.is_visible !== undefined ? f.is_visible : true, // Added visibility mapping
          reconcile: f.reconcile || 'None', // Kept
          use_llm: f.use_llm || false,
          sequence: index
        }));
  ...
  const handleAddField = () => {
    ...
    const newField = {
      ...
      reconcile: 'None', // Kept
      use_llm: true,
      tenant_name: userRole === 'super_admin' ? 'N/A' : null,
  ...
  const handleCardPopupChange = (content) => {
    ...
    // Update the field content separately to avoid nested state updates
    if (cardPopup.index !== null && cardPopup.field) {
      handleFieldChange(cardPopup.index, cardPopup.field, content);
    }
  };
  ...
  const confirmGenerate = async () => {
    ...
    const isSuperAdmin = userRole === 'super_admin';
    // Find the first tenant_id in the list for super_admin, or use user's tenantId
    const targetTenantId = isSuperAdmin ? (fields.length > 0 ? fields[0].tenant_id : null) : tenantId;
    ...
    fieldsToDelete.forEach(field => {
      if (field.vendor_config_id && (isSuperAdmin || field.tenant_id === tenantId)) { // Use tenantId for permission check
        operations.push(
          axios.delete(
            `/api/v1/tables/ap_vendor_field_config?vendor_config_id=eq.${field.vendor_config_id}`, // Use ap_vendor_field_config
            { headers }
          )
        );
      }
    ...
    fields.forEach(field => {
      ...
      // Strip selection_mode from custom_values JSON if present (as requested)
      let finalCustomValues = field.custom_values;
      if (finalCustomValues && (finalCustomValues.includes("Input(") || finalCustomValues.trim().startsWith("{"))) {
        let innerContent = finalCustomValues;
        if (finalCustomValues.includes("Input(")) {
          const match = finalCustomValues.match(/Input\((.*)\)/);
          if (match) innerContent = match[1];
        }

        try {
          const json = JSON.parse(innerContent);
          // Ensure field_name is an array
          if (typeof json.field_name === 'string') {
            json.field_name = json.field_name.split(', ').map(f => f.trim()).filter(Boolean);
          }
          // Strip selection_mode
          if (json.selection_mode) {
            delete json.selection_mode;
          }
          finalCustomValues = JSON.stringify(json);
        } catch (e) {
          // Not a JSON field or legacy format, keep as is
        }
      }
      ...
      if (field.vendor_config_id) {
        const originalField = originalFields.find(o => o.vendor_config_id === field.vendor_config_id);
        const isChanged =
          originalField &&
        ...
        if (isChanged && (isSuperAdmin || (originalField.tenant_id === tenantId))) { // Use tenantId
          const patchUrl = `/api/v1/tables/ap_vendor_field_config?vendor_config_id=eq.${field.vendor_config_id}`; // Use ap_vendor_field_config
          const patchPayload = {
            ...payload,
        ...
        if (isSuperAdmin || tenantId) { // Use tenantId
          const postPayload = {
            ...payload,
            tenant_id: isSuperAdmin ? null : tenantId, // Use tenantId
            created_by: user.user_id
          };
          operations.push(
            axios.post(`/api/v1/tables/ap_vendor_field_config`, [postPayload], { headers }) // Use ap_vendor_field_config
          );
        }
      }
    });
    ...
    try {
      ...
      setShowSaveDialog(false);
      fetchInvoiceConfig(); // Re-fetch the configuration to get new vendor_config_ids
    }
  };
  ...
  }, [fetchInvoiceConfig, fetchAllTenants, userRole]);
    ...
    <>
      ...
      <div className="font-[Poppins] p-4 rounded-xl max-w-auto mx-auto h-[89vh] flex flex-col" data-tour="invoice-config-container">
              ...
              <>
                <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-2 bg-gray-200 p-3 rounded-lg gap-2">
                  <h2 className="text-base font-semibold text-gray-900" data-tour="config-header">
                    Vendor Extraction Table Field Configuration
                  </h2>
                  {userRole === 'super_admin' && tenants.length > 0 && (
                    <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-2">
                      ...
                      <div className="w-full sm:w-48">
                        <SearchableSelect
                          options={['All Tenants', ...tenants.map(tenant => tenant.tenant_name)]}
                          value={selectedTenantId ? tenants.find(t => t.tenant_id === selectedTenantId)?.tenant_name : 'All Tenants'}
                          onChange={(selectedOption) => {
                            if (selectedOption === 'All Tenants') {
                              setSelectedTenantId('');
                            } else {
                ...
                {!tableConfigGenerated && (
                  <div className="flex items-start gap-4 p-2 rounded-lg mb-4 bg-blue-50 text-blue-800 border border-blue-200" data-tour="display-name-note">
                ...
                {!tableConfigGenerated && showGenericFieldsNote && (
                  <div className="flex items-start gap-4 p-5 rounded-xl mb-6 bg-yellow-50 text-yellow-800 border border-yellow-300" data-tour="generic-fields-note">
                    <Lightbulb size={24} className="text-yellow-600 flex-shrink-0 mt-0.5" />
                ...
                {!tableConfigGenerated ? (
                  <div className="flex-grow overflow-hidden relative border border-gray-300 rounded-xl shadow-inner max-h-[70vh]" data-tour="editable-table-wrapper">
                          ...
                          <table className="w-full border-separate border-spacing-0" data-tour="field-config-table">
                            <thead className="sticky top-0 z-20">
                              <tr className="bg-gray-100 shadow-sm">
                                ...
                                <th className="bg-gray-100 font-semibold text-gray-800 uppercase text-[10px] tracking-wide px-2 py-2 text-left border-b-2 border-gray-200 min-w-[120px]" data-tour="field-name">Field Name</th>
                                <th className="bg-gray-100 font-semibold text-gray-800 uppercase text-[10px] tracking-wide px-2 py-2 text-left border-b-2 border-gray-200 min-w-[120px]" data-tour="display-name-column">Display Name</th>
                            ...
                            <SortableContext
                              items={filteredFields.map((field) => field.uniqueId)}
                    ...
                    <div className="absolute inset-0 overflow-auto custom-scrollbar" data-tour="readonly-table-scroll">
                      {filteredFields.length > 0 ? (
                        <table className="w-full border-separate border-spacing-0">
                          ...
                          <tbody>
                            {filteredFields.map((field, idx) => (
                              <TableReadonlyRow key={field.uniqueId} field={field} userRole={userRole} openCardPopup={openCardPopupForReadonly} openCardPopupForReadonly={openCardPopupForReadonly} index={idx} />
                            ))}
                          </tbody>
                        </table>
                      ) : (
                  ...
                  {!tableConfigGenerated ? (
                    <>
                      <button
                        ...
                        disabled={!canEditInvoiceConfig || userRole === 'super_admin'}
                        title={!canEditInvoiceConfig ? editPermissionDeniedMessage : "Add a new field to the configuration"}
                      ...
                      <button
                        ...
                        disabled={!canSave || !canEditInvoiceConfig || !areAllFieldNamesValid()}
                        title={!canEditInvoiceConfig ? editPermissionDeniedMessage : (!areAllFieldNamesValid() ? "Fix invalid field names before saving" : "Save the current invoice structure")}
                        data-tour="save-config-button"
                      >
                        <Save size={14} /> Save Structure
                      </button>
                      <button
                        className="inline-flex items-center gap-1 px-3 py-1.5 rounded-md font-medium text-xs
        ...
        <CardPopup
          ...
          module="vendor" // Hardcoded module for VendorInvoiceConfig
...
export default VendorInvoiceConfig;
